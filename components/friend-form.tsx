...
interface FriendFormProps {
  friend?: Friend | null
  onSubmit: (data: Omit<Friend, "id">) => void
  onCancel: () => void
}
...
export function FriendForm({ friend, onSubmit, onCancel }: FriendFormProps) {
  ...
  return (
    ...
    <Card className="bg-white border-gray-200 p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">{friend ? "Edit Friend" : "Add New Friend"}</h3>
          ...
          <div className="flex items-center">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) => setFormData({ ...formData, dns: e.target.checked })}
                className="w-4 h-4"
              />
              <span className="text-sm font-medium text-gray-700">Do Not Send Receipt</span>
            </label>
          </div>
        </div>
